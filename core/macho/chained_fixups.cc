...
void ForEachFixupsInSegment(
  ...
  for (uint32_t page_idx = 0; page_idx < starts->page_count; ++page_idx) {
    ...
    if (offset_in_page & DYLD_CHAINED_PTR_START_MULTI) {
        ...
        while (!chain_end) {
          chain_end = (starts->page_start[overflow_index] & DYLD_CHAINED_PTR_START_LAST);
          offset_in_page = (starts->page_start[overflow_index] & ~DYLD_CHAINED_PTR_START_LAST);
          auto page_start = image_base.Skip(starts->segment_offset + page_idx * starts->page_size);
...
void ChainedFixupsHelper::Parse() {
  ...
  auto starts_in_image = header
    .Skip(header.As<dyld_chained_fixups_header>()->starts_offset)
  ...
  for (uint32_t seg_idx = 0; seg_idx < starts_in_image->seg_count; ++seg_idx) {
    ...
    const dyld_chained_starts_in_segment* seg_info =
      (dyld_chained_starts_in_segment*) ((uint8_t*) starts_in_image + seg_info_offset);
    
    detail::ForEachFixupsInSegment(image_.base_, seg_info, [this](auto fixup_loc) {
      this->ParseFixup(fixup_loc);
    });
  }
}
